...
```powershell
cd <repo-root>\backend-egovframe-skeleton
...
관리자 mock API는 HTTP Basic 인증을 사용합니다. 프론트는 인증값을 브라우저 세션에만 보관하고, 서버는 `application.yml`의 환경변수로만 값을 읽습니다. 운영 전에는 기본값을 바꾸고, 외부 API 키도 모두 서버 환경변수 또는 Secret Manager로 옮겨 주세요.

```text
ADMIN_INITIAL_LOGIN_ID
...
- 운영 배포와 비밀값 보관 원칙은 [`../docs/deployment-environment.md`](../docs/deployment-environment.md) 를 참고합니다.
