...
-- Keep this file aligned with Flyway V5__staged_upload_approval.sql.

CREATE TABLE IF NOT EXISTS staged_upload (
...
CREATE INDEX IF NOT EXISTS idx_staged_upload_status_created
    ON staged_upload(status, staged_at DESC);
CREATE INDEX IF NOT EXISTS idx_staged_upload_dataset
    ON staged_upload(dataset_key, staged_at DESC);
CREATE INDEX IF NOT EXISTS idx_staged_upload_expires
