...
CREATE INDEX idx_change_request_status_created ON change_request(status, created_at DESC);
CREATE INDEX idx_change_request_target ON change_request(target_type, target_key);
...
CREATE INDEX idx_staged_upload_status_created ON staged_upload(status, staged_at DESC);
CREATE INDEX idx_staged_upload_dataset ON staged_upload(dataset_key, staged_at DESC);
CREATE INDEX idx_staged_upload_expires ON staged_upload(expires_at)
...
CREATE TABLE content_translation (
    ...
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT uq_content_translation UNIQUE (target_type, target_key, field_key, language_code),
    CONSTRAINT ck_content_translation_language CHECK (language_code IN ('ko', 'en', 'ja', 'zh-CN')),
    CONSTRAINT ck_content_translation_status CHECK (translation_status IN ('MISSING', 'DRAFT', 'APPROVED'))
);
