...
- 업로드 대상 반영: 화면 공개 상태이고 업로드 방식에 CSV가 포함된 데이터셋은 미리보기 선택 목록에 표시, 확정 저장은 `facilities`만 허용
...
- 비시설 데이터셋: `stores`, `population`, `air-quality`는 미리보기는 가능하지만 업로드 확정은 차단
...
6. 시설 데이터셋일 때만 필수 필드 매핑이 끝나면 업로드 확정이 가능합니다.
7. 최근 업로드 로그가 쌓입니다.
...
업로드 확정 버튼은 `/api/admin/uploads/commit`을 호출합니다. 백엔드 preview가 성공한 경우 응답의 `uploadId`를 commit 요청에 포함해 실제 업로드 행을 저장합니다. 현재는 `facilities`만 확정 저장이 가능하고, 다른 데이터셋은 미리보기까지만 지원합니다.
