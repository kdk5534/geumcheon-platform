...
> 2026-06-18 이후 UI/UX 전면 개선, 프런트 안정화, 공공데이터 확대, 시각화 계획은
> [`renewal-master-plan.md`](renewal-master-plan.md)와 [`renewal/`](renewal/) 세부 계획을 우선 참고합니다.
> 이 문서는 기존 MVP 구현 이력과 백엔드 단계 진행 상황을 보존합니다.
...
- `stores` 는 `pageNo`/`totalCount` 기반 전체 페이지 수집과 HTTPS 호출을 기본값으로 유지
- 금천구 코드 기반 상가 엔드포인트 전환은 공식 명세 확인 후 후속 작업으로 진행
