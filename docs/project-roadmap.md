...
- 행정동/집계구/상권 경계 레이어 전환
- 반경 분석
- 업종별 밀도/경쟁 지표
- 지도 대체 목록과 접근성 보강
- 선택 권역의 요약/추천 포인트를 보여주는 spotlight 카드
...
- 웹 접근성 점검
- 배포 환경 문서화
- 관리자 데이터셋 편집 폼 오류 안내와 aria-invalid 처리

## 지금 바로 다음 단계

다음은 **P5 지도/분석 고도화**입니다.

단, P3는 PostgreSQL/PostGIS 설치가 필요하므로 계속 보류합니다. 설치 전까지는 API 상태/로그 mock 화면과 관리자 업로드 UX, 지형/분석 화면을 더 다듬을 수 있습니다.
